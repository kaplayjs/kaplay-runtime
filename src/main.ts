...
if ((kapFile.RUNTIME_VERSION ?? RUNTIME_Version) != RUNTIME_Version) {
  k.add([
    k.text(`Illegal Runtime Version: expected v${RUNTIME_Version}, given v${kapFile.RUNTIME_VERSION}`, {
      width: (await getkOpt()).width,
    })
  ])
  throw new Error(`Illegal Runtime Version: expected v${RUNTIME_Version}, given v${kapFile.RUNTIME_VERSION}`);
}
